...
func (s *CA) ActivateCertAuthority(id types.CertAuthID) error {
	...
	if err != nil {
		if errors.Is(err, backend.ErrConditionFailed) {
			return trace.Errorf("failed to activate cert authority %q of type %q due to concurrent modification", id.DomainName, id.Type)
		}
		return trace.Wrap(err)
	}
...
func (s *CA) DeactivateCertAuthority(id types.CertAuthID) error {
	...
	if err != nil {
		if errors.Is(err, backend.ErrConditionFailed) {
			return trace.Errorf("failed to deactivate cert authority %q of type %q due to concurrent modification", id.DomainName, id.Type)
		}
		return trace.Wrap(err)
	}
